...
into a single master file via a union (vertical stack).
...
and two empty columns are added:
    During Marketing Period Quantity
...
and two empty columns are added:
    Before Marketing Period Quantity
    Before Marketing Period Case QTY

Both files end up with the same four marketing-period columns in order:
    Before Marketing Period Quantity
    Before Marketing Period Case QTY
    During Marketing Period Quantity
    During Marketing Period Case QTY
...
Usage:
    1. Set BEFORE_FILE, DURING_FILE, and OUTPUT_DIR below.
...
def create_master_file(before_file, during_file, output_dir, sheet_name=SHEET_NAME):
    ...
    print(f"\nBefore columns: {list(df_before.columns)}")
    print(f"During columns: {list(df_during.columns)}")

    # --- Union (vertical stack) ---
    master = pd.concat([df_before, df_during], ignore_index=True)
    print(f"\nMaster: {len(master):,} rows, {len(master.columns)} columns")
